...
export function checkGeofence(lat: number, lng: number, posJagaList: any[]): GeofenceResult {
  let closest: GeofenceResult = { isInside: false, distance: Infinity, posName: '-', posId: '' };
  for (const pos of posJagaList) {
    ...
    if (dist < closest.distance) {
      closest = { isInside: dist <= (pos.radius || 100), distance: dist, posName: pos.nama || '-', posId: pos.id || '' };
    }
  }
  return closest;
}
...
export function startLocationPing() {
  if (pingInterval) clearInterval(pingInterval);
  sendLocationPing(); // immediate first ping
  pingInterval = setInterval(sendLocationPing, LOCATION_PING_INTERVAL);
  console.log('[Location] Ping started (every 30 min)');
}
